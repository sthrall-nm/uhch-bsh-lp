...
function createLead() {
    return LocalServiceRegistry.createService('ease.http.create.lead', {
         ...
         */
        createRequest: function (svc, contactInfo, requestType) {
            ...
            var easeHelpers = require('*/cartridge/scripts/helpers/easeHelpers');
            var requestBody = easeHelpers.createRequestBody(contactInfo);
            var svcCredential = svc.getConfiguration().getCredential();
            var leadURL = svcCredential.URL + '*';
            svc.setURL(leadURL);
