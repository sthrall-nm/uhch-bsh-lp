...
var querystring = function (raw) {
    ...
    if (raw && raw.length > 0) {
        var qs;
        if (raw.indexOf('orderNo') === 0) {
            // handle condition for the order number encryption
            qs = raw.substring(raw.indexOf('?') + 1).split('&');
        } else {
            qs = raw.substring(raw.indexOf('?') + 1).replace(/\+/g, '%20').split('&');
        }
