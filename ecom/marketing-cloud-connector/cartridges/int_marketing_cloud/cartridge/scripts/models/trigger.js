...
function newMessage(data){
    ...
    //Customization for sending name
    var Site = require('dw/system/Site');
    let fromName = Site.current.getCustomPreferenceValue('mcFromName');

    msg.setFrom(data.fromEmail,fromName).setTo(toEmail);
    //Customization for sending subscriber key from global customer
    var subscriberKey = customer && customer.profile && customer.profile.custom && 'sfdcContactID' in customer.profile.custom ? customer.profile.custom.sfdcContactID : toEmail;
    msg.setSubscriberKey(subscriberKey);
